...
使用方法:
    ...
    # 带话题标签发布
    python publish_xhs.py --title "标题" --desc "描述" --images *.png --topics "游戏" "游戏行业"
...
def search_topics(client, keywords: List[str]) -> List[Dict[str, Any]]:
    """搜索话题并返回话题列表
    ...
    for keyword in keywords:
        try:
            print(f"🔍 搜索话题: {keyword}")
            results = client.get_suggest_topic(keyword)
            ...
            if results:
                ...
                if topic['id'] not in seen_ids:
                    topics.append({
                        "id": topic['id'],
                        "name": topic['name'],
                        "type": topic.get('type', 'official')
                    })
                    seen_ids.add(topic['id'])
                    print(f"  ✓ 找到话题: {topic['name']}")
                else:
                    print(f"  - 跳过重复话题: {topic['name']}")
            else:
...
class LocalPublisher:
    ...
    def init_client(self):
        ...
        # 解析 cookie 获取 a1
        cookie_dict = parse_cookie(self.cookie)
        a1 = cookie_dict.get('a1', '')
        
        # XhsClient 调用 external_sign(url, data, a1=a1, web_session=web_session)
        # 需要返回 {"x-s": ..., "x-t": ..., "x-s-common": ...}
        # local_sign 签名: (uri, data=None, ctime=None, a1='', b1='')
        def sign_func(url, data=None, a1='', web_session=''):
            # web_session 作为 b1 参数传入
            return local_sign(url, data, a1=a1, b1=web_session)
        
        self.client = XhsClient(cookie=self.cookie, sign=sign_func)
        self._a1 = a1  # 保存 a1 用于调试
    ...
    def publish(self, title: str, desc: str, images: List[str], 
        ...
        if topics:
            for t in topics:
                tag_text = f" #{t['name']}[话题]#"
                # 避免重复添加同一个标签
                if tag_text not in desc:
                    desc += tag_text
        try:
...
class ApiPublisher:
    ...
    def __init__(self, cookie: str, api_url: str = None):
        ...
        self.client = None  # 用于话题搜索
        
    def init_client(self):
        """初始化 API 客户端"""
        print(f"📡 连接 API 服务: {self.api_url}")
        
        # 同时初始化本地 XhsClient 用于话题搜索
        try:
            from xhs import XhsClient
            from xhs.help import sign as local_sign
            ...
            print(f"⚠️ 无法初始化本地客户端（话题搜索可能不可用）: {e}")
    ...
    def publish(self, title: str, desc: str, images: List[str], 
        ...
        print(f"  🖼️ 图片数量: {len(images)}")
        if topics:
            print(f"  🏷️ 话题: {', '.join([t['name'] for t in topics])}")
...
示例:
  ...
  # 带话题标签发布
  python publish_xhs.py -t "我的标题" -d "正文内容" -i *.png --topics "游戏" "游戏行业"
...
'''
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help='话题标签关键词（可以多个，如 --topics "游戏" "游戏行业"）'
    )
    parser.add_argument(
        '--private',
        action='store_true',
    ...
    if args.topics:
        try:
            topics = search_topics(publisher.client, args.topics)
        except Exception as e:
            print(f"⚠️ 搜索话题失败: {e}")
            print("将继续发布，但不带话题标签")
            topics = None
